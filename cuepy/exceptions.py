...
class IncompatibleProtocol(Exception):
    """
    if developer is calling the function that is not supported by the server(either because protocol
    has broken by server or client or because the function is new and server is too old. Check
    CorsairProtocolDetails for details) (developer error)
    """
    pass
...
error_id = {
    1: ServerNotFound,
    2: NoControl,
    3: ProtocolHandshakeMissing,
    4: IncompatibleProtocol,
    5: InvalidArguments
}
